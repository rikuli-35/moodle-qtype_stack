...
もし文字列一致の判定が予期せず失敗した場合は、テストページでPRTが実際に何を比較しているかを確認する。その上で、比較対象に合うように教師側の正解を修正する必要がある。
...
またこの解答欄を使用すると、教師はデバッグ用に整形された状態のJSONオブジェクトを確認できるようになる。
...
他の解答欄形式との一貫性のために、教師は正解を設定しておく必要がある（例：空文字列 `""` など）。ただし、ここで設定した回答が採点などで使用されることはない。
