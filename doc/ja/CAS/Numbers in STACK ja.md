...
1.[数値の解答テスト](../Authoring/Answer_Tests/Numerical.md)、
2.[複素数](Complex_numbers.md).
3.[数値の四捨五入](Numerical_rounding.md).
...
[平方根の表示](../Authoring/Question_options.md#surd)についてはオプションで変更可能である。

同様に、複素数は1つのまとまりとしてではなく、実部と虚部の和として、あるいは指数関数を介して表現される。
複素数の入力と表示は、数学、物理学、工学の間で記号が異なるため、意味と表示を整理するために、[sqrt(-1)](../Authoring/Question_options.md#sqrt_minus_one)というオプションが各問題に設定されている。
...
学生が`3e4`（これは$3\times 10^{4}$を意味する）のような科学記数法を使用することを想定している場合、[strict syntax オプション](../Authoring/Inputs/index.md#Strict_Syntax) を使用するとよい。

[数値の四捨五入](Numerical_rounding.md)については、別のドキュメントを参照すること。
...
単位入力は別として、他のすべての入力タイプでは浮動小数点数の不要な末尾の0が表示時に切り捨てられ、有効数字に関する情報を失われる。したがって、学生の解答が浮動小数点数の場合、末尾の0は表示されない。特に、有効数字を評価したい場合は、教師側の解答に単位をつけずに[単位入力タイプ](../Topics/Units.md)を使用すること。単位入力タイプは学生が入力したものと同じ有効数字の桁数を表示する。
...
`decimalplaces(x,n)`および`dispdp(x,n)`は四捨五入を行う。詳細については、[四捨五入](Numerical_rounding.md)に関する資料を参照すること。いくつか例外がある。
