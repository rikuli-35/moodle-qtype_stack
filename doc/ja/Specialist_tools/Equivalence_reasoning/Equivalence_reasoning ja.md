...
同値変形（reasoning by equivalence）とは、行ごとの代数的推論のことです。

同値変形とは、代数式、または式の中の項を同値なものに順次置き換えていき、「解かれた」形に到達するまで繰り返す反復的な形式的記号操作のことです。同値変形は初等数学において非常に一般的です。同値変形が課題のすべてである場合（例えば二次方程式を解く場合）もあれば、より大きな問題の重要な一部分である場合もあります。例えば、帰納法のステップの証明は、同値変形によって達成されることが多いです。

（i）同値な式の書き換え、（ii）方程式の求解の２つの方法があります。
...
\\end{array}\\]
この形式の推論では、式の先頭に等号を記述する必要はありません。
...
STACKには、どちらの推論形式が使用されているかを判別する述語関数があります。この述語は式のリストに対して適用されます。
...
STACKでは、隣接する式の同値性は `AlgEquiv` 評価関数で判定されます。

* \\(\\color{green}{\\checkmark}\\)（`CHECKMARK`）は、後続の**式**が前の式と同値であることを示すために使用されます。
  例：\\[\\begin{array}{lll} & x^2-x-30& \\\\ \\color{green}{\\checkmark}& =\\left(x-6\\right)\\cdot \\left(x+5\\right)\\end{array}\\]
* \\(\\color{red}{?}\\)（`QMCHAR`）は、ある式が次の式と同値でないことを示すために使用されます（方程式または式のいずれの場合にも適用）。

## 同値な方程式

2つの方程式が同値であるとは、それらが同じ解集合を持つことを意味します。

方程式を用いた行ごとの代数的推論には、異なる種類のオブジェクトが関わります。

1. 代数方程式：（暗黙的に）解の集合を表します。例：\\(x^2+6=5x\\)
2. 明示的な解を与える論理式：例：\\(x=2 \\mbox{ or } x=3\\)
3. 解を表す数の集合：例：\\(\\{2,3\\}\\)、[区間](../../CAS/Real_Intervals.md)として \\([0,\\infty)\\)（STACKでは `co(0,inf)`）
4. 不等式のシステム：例：\\( x\\geq 0\\)

同値変形とは、通常、与えられた方程式から明示的な解へと導く過程のことです。

2つの方程式が「同じ」であるかどうかを判断する際には、いくつかの選択が必要です。

1. 実数上で作業しているのか、複素数上なのか、それとも他の体系か。
2. 重複する解をどう扱うか。例えば、\\((x-2)^3=0\\) と \\(x=2\\) は同値な方程式か。

方程式を用いた推論では、いくつかのエッジケースがあります。

* \\( \\mbox{false} = \\{\\} = \\mbox{none}\\) は、\\(1=0\\) のような方程式のように、解が空である状況を表します。
* \\( \\mbox{true} = \\mbox{all}\\) は、\\(x=x\\) のような常に真となる自明な方程式を表します。なお、アトム `all` は \\(\\mathbf{C}\\) ではなく \\(\\mathbf{R}\\) として表示されます（これは `texput(all, "\\\\mathbb{R}");` で変更可能です）。

これらのエッジケースは `AlgEquiv` の動作とは異なります。`AlgEquiv` では \\(\\{\\}\\) を \\( \\mbox{false}\\) とは見なしません。

なお、STACKには「ステップサイズ」の概念はありません。将来的には、2つの式や方程式間の「距離」を測定することが計画されています。教師はこの距離を用いて、ステップが大きすぎるか、あるいは論証にもう1つ中間ステップが必要かを判断できるようになります。

* \\(\\color{green}{\\Leftrightarrow}\\)（`EQUIVCHAR`）は、後続の**方程式**が前の方程式と同値であることを示すために使用されます。
  例：\\[\\begin{array}{cc} \\  & x^2-x-30=0 & \\\\ \\color{green}{\\Leftrightarrow} & \\left(x-6\\right)\\cdot \\left(x+5\\right)=0\\end{array}\\]
* \\(\\color{red}{?}\\)（`QMCHAR`）は、ある式が次の式と同値でないことを示すために使用されます（方程式または式のいずれの場合にも適用）。
* \\(\\color{green}{\\text{(Same roots)}}\\)（`SAMEROOTS`）は、重複度を考慮しない同じ根の集合を示すために使用されます。
...
* \\(\\color{red}{\\Rightarrow}\\)（`IMPLIESCHAR`）と \\(\\color{red}{\\Leftarrow}\\)（`IMPLIEDCHAR`）は、式の解集合が部分集合の関係にある場合に使用されます。\\(P\\) を \\(p(x)=0\\) の解集合、\\(Q\\) を \\(q(x)=0\\) の解集合とし、\\(P\\subset Q\\) であるとき、\\(p(x) \\color{red}{\\Rightarrow} q(x)\\) と記述します。
...
* \\(\\color{green}{\\Leftrightarrow}\\, \\color{blue}{(\\mathbb{R})}\\) は、実数上で解く場合に使用されます（オプション `assume_real` を参照）。
...
* \\(\\color{blue}{\\text{Assume +ve vars}}\\) は、正の実数上で解く場合に使用されます（オプション `assume_pos` を参照）。
...
* \\( \\color{green}{\\log(?)} \\) は、規則 \\( A=B \\Leftrightarrow e^A=e^B\\) を使用して同値性が成立する場合に使用されます。
...
* \\(\\color{red}{\\text{Missing assignments}}\\) は、学生が変数の記述を忘れた場合に使用されます。
  例：\\[{\\begin{array}{lll} &x=1\\,{\\text{ or }}\\, x=2& \\cr \\color{red}{\\text{Missing assignments}}&x=1\\,{\\text{ or }}\\, 2& \\cr \\end{array}}\\]
* \\(\\color{red}{\\text{and/or confusion!}}\\) は、学生がand/orを誤って使用した場合に使用されます。
...
* \\(\\color{blue}{\\int\\ldots\\mathrm{d}x}\\) と \\(\\) は、学生が微積分の操作を行ったと推定される場合に使用されます（オプション `calculus` を参照）。
