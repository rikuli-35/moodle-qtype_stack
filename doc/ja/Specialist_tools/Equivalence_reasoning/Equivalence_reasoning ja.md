...
同値変形（reasoning by equivalence）とは、行ごとの代数的推論のことである。
...
STACKには、どちらの推論形式が使用されているかを判別する述語関数がある。この述語は式のリストに対して適用される。
...
方程式を用いた行ごとの代数的推論には、異なる種類のオブジェクトが関わる。
