...
解答欄には、カスタムバリデーションテスト（解答の有効性に関する検証テスト）と応答メッセージを設定できる。これらのいわゆる[カスタムバリデータ](../../CAS/Validator.md)は、受け取った入力に対して動作し、学生に表示される出力を生成できる。この例題では、問題にバリデータとフィードバックジェネレータの両方が含まれている。バリデータは、入力がxの関数であり、それ以外のものでないことを検証する（プロットが可能であるかを確認するため）。フィードバックジェネレータは、あらかじめ決められた範囲内の200点で入力を評価し、それらの点をJSON形式でエンコードした隠し `<div>` を出力する。
